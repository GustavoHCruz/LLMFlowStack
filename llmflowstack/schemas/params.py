...
class TrainParams:
    ...
    warmup_steps: int = 0
    lr: float = 2e-5
    optim: Literal["adamw_torch", "adamw_torch_fused", "sgd"] = "adamw_torch"
    logging_steps: int = 1
...
class GenerationParams:
    ...
    def __post_init__(self):
        ...
        if self.mode == "greedy":
            ...
            self._ensure_none(
                "sample", ["num_beams", "length_penalty", "early_stopping"]
            )
            ...
            self._ensure_none("beam", ["temperature", "top_p", "typical_p"])
    ...
    def _ensure_none(self, mode: str, fields: list[str]) -> None:
        for field in fields:
            if getattr(self, field) is not None:
                raise ValueError(f"Field '{field}' is not allowed when mode='{mode}'")
