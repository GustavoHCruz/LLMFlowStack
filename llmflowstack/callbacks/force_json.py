...
JsonPhase = Literal[
    "expect_value", "expect_key_or_end", "expect_colon", "expect_comma_or_end", "done"
]
...
class JsonPrefixValidator:
    ...
    def _current_phase(self) -> JsonPhase:
        if self.stack:
            return self.stack[-1].phase
        return self.top_phase

    def _set_phase_after_value(self) -> None:
        if self.stack:
            self.stack[-1].phase = "expect_comma_or_end"
        else:
            self.top_phase = "done"

    def _set_phase_after_open_obj(self) -> None:
        self.stack.append(_Container("obj", "expect_key_or_end"))

    def _set_phase_after_open_arr(self) -> None:
        self.stack.append(_Container("arr", "expect_value"))

    def _close_container(self, close_char: str) -> bool:
        if not self.stack:
            return False
        top = self.stack[-1]
    ...
    def _starts_number(self, ch: str) -> bool:
        return ch.isdigit() or ch == "-"

    def _consume_number(self, s: str, i: int) -> int:
        start = i
        n = len(s)
    ...
    def _consume_literal(self, s: str, i: int) -> int:
    ...
    def validate_and_update(self, appended: str) -> bool:
        if not appended:
            return True
        ...
        while i < n:
            ...
            if self.in_string:
                ...
                if ch == '"':
                    ...
                    if (
                        self.stack
                        and self.stack[-1].typ == "obj"
                        and phase == "expect_key_or_end"
                    ):
            ...
            if ch == ":":
                if (
                    not self.stack
                    or self.stack[-1].typ != "obj"
                    or phase != "expect_colon"
                ):
                    return False
...
class ForceJsonLogitsProcessor(LogitsProcessor):
    def __init__(self, tokenizer, top_k: int = 256) -> None:
    ...
    def _sync_prefix(self, input_ids: LongTensor) -> None:
        seq = input_ids[0]
    ...
    def __call__(self, input_ids: LongTensor, scores: FloatTensor) -> FloatTensor:
        ...
        for tid in candidate_ids:
            ...
            self.validator.stack = [
                _Container(d["typ"], d["phase"]) for d in stack_dicts
            ]
...
class StopOnJsonComplete(StoppingCriteria):
    def __init__(self, processor: ForceJsonLogitsProcessor) -> None:
