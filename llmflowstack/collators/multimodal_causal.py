...
class MultimodalCausalCollator:
    ...
    def _get(self, x: Any, key: str) -> Any:
    ...
    def __call__(self, batch: list[Any]) -> dict[str, torch.Tensor]:
        ...
        for item in batch:
            ...
            if ids is None or attn is None or labels is None:
                raise ValueError(
                    "Each item must provide input_ids, attention_mask, and labels."
                )
        ...
        if has_pixels:
            if any(p is None for p in pixels):
                raise ValueError(
                    "Mixed batches with and without pixel_values. Use a separate dataloader or ensure consistency."
                )
